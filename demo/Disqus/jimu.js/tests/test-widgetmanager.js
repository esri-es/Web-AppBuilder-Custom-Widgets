require(["doh/runner",
 ...
 'dojo/NodeList-dom'],
		...
		runTest: function () {
			...
			wm.loadWidget(widgetConfig).then(function(widget) {
				dohDeferred.getTestCallback(function(widget) {
					...
					doh.assertEqual('/portal/apps/webappbuilder/stemapp/jimu.js/tests/testwidgets/Widget1/', widget.folderUrl);
					doh.assertEqual('Widget1', widget.name);
			...
			wm.loadWidget(widgetConfig1).then(function(widget1){
				wm.loadWidget(widgetConfig2).then(function(widget2){
					dohDeferred.getTestCallback(function(){
						...
						doh.assertEqual(5, query('link').length);
						doh.assertEqual('/portal/apps/webappbuilder/stemapp/jimu.js/tests/testwidgets/Widget1/css/style.css', node.attr('href')[0]);
					})();
				});
			});
			return dohDeferred;
	...
	{
		name: 'test widget upgrade',
		runTest: function (){
			var dohDeferred = new doh.Deferred(),
				wm = WidgetManager.getInstance(),
				widgetConfig1 = {
					"id": "testwidget3",
					"uri": "jimu/tests/testwidgets/Widget-upgrade/TestWidget3",
					"version": '1.0'
				};
